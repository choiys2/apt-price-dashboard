...
| 5 | 고도화 1차: 면적 타입 정규화 · 신고가 탐지 · 거래유형 분리 · 전세가율 · 누락 탐지 | 완료 |
| 6 | 고도화 2차: 자동 재시도 · 분위수 밴드 · 단지 드릴다운 · 층 프리미엄 | 완료 |
| 7 | 고도화 3차: 예산 역질의 · 주간 자동 브리핑 | 완료 |
...
| `weekly_brief.py` | 대시보드를 열지 않아도 읽히는 주간 요약 (`reports/latest.md`) |
| `test_parse.py` / `test_analytics.py` / `test_brief.py` | 단위 테스트 76개 (네트워크 불필요) |
| `.github/workflows/probe.yml` | API 실측 확인용 수동 워크플로 |
| `.github/workflows/collect.yml` | 수집 시도 1회 (재사용 워크플로) |
| `.github/workflows/deploy.yml` | 매일 06:00 KST 자동 갱신·배포, 실패 시 새 러너로 최대 3회 재시도 |
...
대시보드의 기본 방향은 지역 → 가격이다("강남구는 평당 1억"). 실사용자의 질문은 반대라서,
가격 → 지역으로 뒤집는 화면을 따로 뒀다. 예산·최소 전용면적·지역을 넣으면 가능한 단지가
전용면적 넓은 순으로 나온다.

- 재료는 **단지 × 전용타입별 시세 인덱스**(최근 9개월, 거래 3건 이상, 중개거래만).
  직거래는 시세보다 28.5% 낮게 신고되는 경우가 많아 "이 값이면 살 수 있다"는 표에 넣지 않는다.
- 13,000행에 딕셔너리 키를 매번 실으면 JSON 이 3MB 를 넘어(키 이름만 행당 110바이트)
  배열로 눕히고 지역명은 코드→이름 표로 한 번만 싣는다. 3.1MB → 1.06MB.
- 시세는 그 기간 **중위 거래가**이지 호가가 아니다.
...
- **전세가율은 같은 단지 × 같은 전용타입끼리 짝지어 계산한다.** 지역 단위로 중위 전세금을
  ...
  몰리는 식) 실제와 크게 어긋난다.
- **대표값은 중위 하나로 부족하다.** 사분위(25/75%)를 함께 낸다. 실측으로 강남구는
  25% 7,514 / 중위 10,909 / 75% 14,217 이라 사분위폭이 중위의 61%다.
- **층 프리미엄은 단지 × 타입 안에서 잰다.** 전체 평균으로 저층·고층을 비교하면 +12.6%가
  나오지만, 고층 단지가 대체로 신축이라는 효과가 섞인 값이다. 같은 단지 같은 타입 안에서
  중위 대비 편차로 재면 저층 -3.6% / 고층 +0.8% 로 **총 4.4%p**다. 3배 부풀려진 셈이다.
